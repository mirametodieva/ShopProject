...
<%
	Data dataDairyProduct = new Data();
	List <DairyProduct> dp = dataDairyProduct.getDataDairyProduct();
	
	String id = request.getParameter("id");
	DairyProduct pr = dataDairyProduct.getDairyProduct(Integer.parseInt(id));
	...
	ShoppingCart.addDairyProduct( pr, sellQuantity );
		}
...
body {
	background-image: url(images/Background/5.1.jpg);
}
</style>
...
<body>
	...
	<div class="page">
		...
		<div class="path">
			<ul>
				...
				<li><a href="DairyProduct.jsp"><%=pr.getPath()%></a></li>
		...
		<div class="buttons">
			<form action="productDairyProductDetails.jsp?id=<%=pr.getId()%>"
				method="post">
	...
	<div class="right-content">
		...
		<%
			for( int i=1;i<5;i++){
		%>
		<a href="productDairyProductDetails.jsp?id=<%=dp.get(i).getId()%>"><img
			src="<%=dp.get(i).getPictureName()%>"
			title="<%=dp.get(i).getType()%>"><br> <%=dp.get(i).getType()%></a>
		<%
			}
		%>
